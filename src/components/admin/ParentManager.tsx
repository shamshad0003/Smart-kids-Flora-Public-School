...
export default function ParentManager({ parents }: { parents: Parent[] }) {
    ...
    const handleCreate = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        startTransition(async () => {
            const res = await createParent(fd) as { error?: string, success?: boolean, tempPassword?: string };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => { setShowForm(!showForm); setEditing(null); setError(""); setNewAccountInfo(null); }}
                    className="flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-lg shadow-pink-200">
                ...
                <div className="bg-emerald-50 border-2 border-emerald-100 rounded-[2rem] p-6 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="font-bold text-emerald-900">Parent Account Created!</h3>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl p-4 border border-emerald-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Parent Email</p>
            ...
            {showForm && !newAccountInfo && (
                <div className="bg-white border border-slate-200 rounded-[2rem] shadow-xl shadow-slate-200/50 p-8 animate-in zoom-in-95 duration-300">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-black text-slate-900">Register Parent</h3>
                    ...
                    <form onSubmit={handleCreate} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div className="space-y-1">
                                ...
                                <input name="fullName" placeholder="Robert Smith" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 focus:bg-white transition-all font-medium" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 ml-1 uppercase">Email Address</label>
                                <input name="email" type="email" placeholder="robert.smith@email.com" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 focus:bg-white transition-all font-medium" />
                            </div>
                            <div className="space-y-1 md:col-span-2">
                                <label className="text-xs font-bold text-slate-500 ml-1 uppercase">Phone Number</label>
                                <input name="phone" placeholder="+1 234 567 890" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 focus:bg-white transition-all font-medium" />
                            </div>
                        </div>
                        <div className="bg-pink-50 p-4 rounded-2xl border border-pink-100 text-xs text-pink-700 font-medium">
                            A secure temporary password will be generated for this parent account. Parents can link to their children after logging in or via the student management panel.
                        </div>
            ...
            {editing && (
                <div className="bg-white border border-pink-200 rounded-[2rem] shadow-xl shadow-pink-100/50 p-8 animate-in zoom-in-95 duration-300">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-black text-slate-900">Edit Parent Profile</h3>
                    ...
                    <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-slate-500 ml-1 uppercase">Full Name</label>
                            <input name="fullName" defaultValue={editing.fullName} required className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 transition-all font-medium" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-slate-500 ml-1 uppercase">Phone Number</label>
                            <input name="phone" defaultValue={editing.phone ?? ""} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 transition-all font-medium" />
                        </div>
                        <div className="md:col-span-2 flex justify-end pt-2">
                            <button type="submit" disabled={isPending}
                                className="w-full md:w-auto flex items-center justify-center gap-2 bg-pink-600 hover:bg-pink-700 text-white px-8 py-3.5 rounded-2xl font-bold text-sm transition-all shadow-lg active:scale-95 disabled:opacity-60">
            ...
            {parents.length === 0 ? (
                <div className="bg-white rounded-[2rem] border border-slate-100 px-6 py-20 text-center shadow-sm">
                    ...
                    <p className="text-slate-500 font-bold text-lg">No parents yet</p>
                    <p className="text-slate-400 text-sm mt-1">Register parents to link them with their children&apos;s accounts.</p>
                </div>
            ) : (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50/50 border-b border-slate-100">
                                <tr>
                                    {["Parent", "Contact", "Status", "Children", "Actions"].map(h => (
                                    ...
                                    <tr key={p.id} className="group hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="font-bold text-slate-900 group-hover:text-pink-600 transition-colors">{p.fullName}</span>
                                        ...
                                        <td className="px-6 py-4">
                                            <button 
                                                onClick={() => handleToggleStatus(p.id, p.isActive ?? true)}
                                                disabled={isPending}
                                            ...
                                            >
                                                {p.isActive !== false ? <UserCheck className="h-3 w-3" /> : <UserX className="h-3 w-3" />}
                                                {p.isActive !== false ? 'Active' : 'Inactive'}
                                            </button>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="bg-pink-50 text-pink-700 px-3 py-1 rounded-lg text-xs font-extrabold border border-pink-100">
                                                {p._count.children} Student{p._count.children !== 1 ? 's' : ''}
                                            </span>
                                        </td>
